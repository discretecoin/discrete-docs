...
\vspace{0.7cm}
{\normalsize Pre-launch protocol paper, revision 1.0 --- July 2026\par}
...
Mining uses DiscretePower-2: every candidate is signed by the coinbase
identity's ML-DSA key, and that raw signature tape is injected throughout a
modified yespower (\code{yespower-dp2}) memory-hard core, with the signature
...
\item[Honest about what is and is not protected.] The monetary policy has no
  hard cap (by design, Section~\ref{sec:monetary}); mining is CPU-egalitarian
  and therefore not botnet-proof (Section~\ref{sec:spow}); finality trades a
  recoverable liveness events for protection of already-observed deep history
...
Proof-of-work & DiscretePower-2 & FIPS 202/204 + yespower-dp2 & 16\,MiB scratchpad (draft) \\
...
single 32-byte master seed (BIP-39 mnemonic or CSPRNG output). The two
...
authority. Backing up the one master seed reconstructs the full post-quantum
identity; recovery is a deterministic chain rescan
(Section~\ref{sec:scanning}) with no per-output key material ever stored.
...
\midrule
\code{version}       & $= 1$                                & 1\,B \\
\code{networkPrefix} & varint network discriminator          & 1--2\,B \\
...
$(\mathit{blockHeight}, \mathit{txIndex})$ --- making it immutable, globally
unique, and resolvable by any node with no extra trust. It renders in
\textbf{H-I-C} form,
\[
\underbrace{\mathit{height}}_{H} \text{-} \underbrace{\mathit{txIndex}}_{I}
\text{-} \underbrace{c}_{\mathrm{check}},
\]
where $c$ is a Luhn mod-36 check character over the concatenated decimal
digits of $H$ and $I$, so any single transcription typo is caught locally
before a lookup. Registration persists in the node database under two
...
\end{aligned}
\end{equation}
where both public keys are bound so one proof cannot be replayed with different
spend identities, and $\mathit{refBlockHash}$ must be a main-chain block within the last
...
key state; Discrete instead binds a 64-bit \textbf{routing index} $T$ into
each output's delivery. $T$ is not a key: spend authority is unchanged, and
the index is recovered from the output itself during normal decapsulation ---
no lookup table, no extra scan pass. A deposit identifier renders as
\textbf{H-I-T-C} (the base account plus index plus a Luhn check character over
all three fields), which can never alias the two-dash base form. One account
can therefore hand every counterparty its own deposit address and recover all
of them from the single master seed.
...
\item \textbf{Spends are publicly linkable.} Spending reveals
  $\mathit{spk}$, so all spends by one identity are linkable, and the spent
  output is identified exactly. This is the deliberate, documented Phase-1
  trade-off: the transaction graph is transparent
...
\begin{algorithm}[tbp]
\caption{\textsc{Scan} --- recipient side, per transaction $\mathit{tx}$ with wallet keys $(\mathit{vsk}, \mathit{spk})$.}
\label{alg:scan}
\begin{lstlisting}[style=pseudo]
function ScanTx(tx, vsk, spk):
    ...
    for each (i, out) in tx.outputs where out.type == 0x10:
        ...
        for T known or T = 0:      // recovered T below confirms routing
...
tag of 3{,}137 bytes --- fits without surcharge. The maximum extra surcharge
produces a 0.10-XDS floor. Input/output counts and total transaction size remain
...
A payer can prove, after the fact and without interaction, that specific
outputs of a transaction commit to a recipient's spending authority. The
sender retains each output's $\rho$; the proof artifact
\[
\bigl(\mathrm{version}{=}2,\; \mathit{genesisId},\; \mathit{txid},\;
H(\mathrm{domain} \concat \mathit{spk}),\;
\{(i, \rho_i)\}\bigr)
\]
lets any verifier recompute Eq.~\eqref{eq:spendcommit} against the claimed
$\mathit{spk}$ for each listed output of the canonical transaction and sum
the public amounts; forging an entry for a foreign output would require a
SHA3-256 second preimage. The proof deliberately does \emph{not} claim
ML-KEM delivery: FIPS~203 forbids exposing encapsulation randomness through
application interfaces, so no unilateral, non-interactive proof of view-key
delivery is possible with the standard API --- a fact the design states
rather than papers over. Where proof of actual receipt is required, the
recipient signs a domain-separated receipt with its spend key (an
interactive but fully standard flow reserved as a future wallet surface).
...
Discrete's proof-of-work is \textbf{DiscretePower-2}, a composition of
SHAKE-256, ML-DSA-65, and \code{yespower-dp2} --- yespower 1.0 with the raw
signature tape injected throughout its memory-hard loops.
...
Y_B &= \mathrm{yespowerDP2}_{N=4096,\,r=32}(H_B;\mathrm{pers}=P;\mathrm{tape}=\mathit{tape}_B) \\
...
carried in the coinbase \code{extra} \emph{before} any \code{yespower-dp2} work runs,
...
signature, and the whole $3{,}312$-byte tape is re-injected before every
memory-store and memory-index step of the core, so no memory-hard prefix is
...
\code{yespower-dp2} collapses to stock yespower 1.0.
...
function Mine(template B, difficulty D, spend keys (spk, ssk)):
    while not stopped:
        if node is syncing or has no peers: pause  // stop-on-isolation, Sec. 6.4
        ...
        Y $\leftarrow$ yespower-dp2$_{4096,32}$(H, pers=SHAKE256(dP, 32), tape)
...
\item \textbf{ASIC resistance.} \code{yespower-dp2}'s 16-MiB scratchpad
  ($N{=}4096$, $r{=}32$, draft) is the memory-hardness mechanism, keeping the
  algorithm CPU-egalitarian. Because it is a new yespower-derived algorithm, its
  hardware ratios and final parameters require independent benchmarking before
  mainnet. The personalization tag is fixed and Discrete-specific, so
...
that caps the downward step. The future-time limit is $7T$ and the median-of-60
past-timestamps rule applies, the standard companions that keep timestamp
manipulation unprofitable at this window size.
...
\item \textbf{Prevention in code.} The built-in miner pauses when the node is
  syncing and stops outright when all peers disconnect
  (Algorithm~\ref{alg:mine}), so the most common wedge source --- the lone
  disconnected miner --- waits and resyncs cleanly instead of building a
  divergent fork. This turns an operator-discipline recommendation into a
  code-enforced guarantee for total disconnection in the reference miner; it
  does not detect every partition or eclipse that retains stale peers.
...
and solves it well --- but a pool of any kind works by sharing each block's
reward across recent contributors, and sharing work \emph{is} delegating
work: it is precisely what SPoW forbids, and the two rules compete for the
same coinbase. Nor can a share-chain be rescued by consensus membership rules
...
,\;
\underbrace{\Bigl\lfloor \frac{0.02\, A}{B} \Bigr\rfloor}_{\text{perpetual tail}}
\;\Bigr),
\label{eq:emission}
\end{equation}
(with the exponential term floored at a vestigial constant once $A \ge S$;
...
$2^{-18} \approx 1/262{,}144$ of the remaining distance to $S$. The initial
reward is $\lfloor S/2^{18}\rfloor = 8{,}010$ atoms $= 80.10$~XDS. Supply
follows $A(t) \approx S\,(1 - e^{-t/\tau})$ with time constant
...
exponential when $A \approx 0.9853\,S \approx 20.69$ million XDS, roughly
3.2 years after launch, at a per-block reward of $\approx 1.19$~XDS. From
...
\addplot[color=kwcolor, thick, domain=0:12, samples=300]
  { max( 80.10 * exp(-x/0.7481), 1.1986 * (1 - exp(-x/0.7481)) * exp(0.02*x) ) };
...
\end{axis}
\end{tikzpicture}
\caption{Per-block reward over time: smooth exponential decay from 80.10~XDS
(reward half-life $\approx$ 6 months) into the perpetual $\sim$2\%/yr tail,
which crosses over at $\approx$ 20.69M~XDS supply ($\approx$ 3.2 years) and
...
Initial block reward & 80.10 XDS \\
...
\item It is not liquid at launch. It vests as \textbf{21 batches of
  50{,}000 XDS}, one unlocking each quarter over five years, using the
...
\item The genesis coinbase \code{extra} embeds a dated headline ---
  ...
  Chancellor headline and a statement of purpose. The fixed block timestamp is
  22~June 2026, earlier than this headline. Neither timestamp nor headline
  proves the public announcement time or absence of private mining; the
  headline only lower-bounds when the candidate artifact could have been
  assembled.
\end{itemize}
...
monotonization, the $N^2T/20$ floor on $L$, and the $7T$ future limit constrain
timestamp leverage. Adversarial simulation is still required before launch.
...
Section~\ref{sec:finality}. The reference miner stops at zero peers, but an
eclipsed miner with stale peers or a connected minority segment can continue.
...
DiscretePower-2 identity-bound work; a local depth-10 reorganization limit; and an
...
Block target & 90 s \\
Proof-of-work & DiscretePower-2: SHAKE-256 + ML-DSA-65 + yespower-dp2 (signature-tape), $N{=}4096$, $r{=}32$ (16 MiB, draft) \\
Mining binding & coinbase recipient $=$ block signer (ML-DSA-65), Eq.~\eqref{eq:rhoc} \\
Difficulty & LWMA-1, window $N{=}60$, no cut/lag; future-time limit $7T$ \\
...
Fees & flat $0.01$ XDS + extra surcharge, Eq.~\eqref{eq:fee}; max $0.10$ XDS \\
Free registration & identity-bound yespower PoW, target $\mathtt{0x00007FFF FFFFFFFF}$ ($2^{17}$ exp.), window 60, $\le$100/blk \\
Transaction relay & Dandelion++ (epoch 600 s, 2 stems, $p{=}0.9$, embargo 173 s) \\
Default ports & P2P 9330; RPC 9331 \\
Genesis timestamp & 2026-07-13 02:39:16 UTC \\
Genesis hash & {\scriptsize\texttt{3101b248ac612883c47b93fa6a4d9d34020ee2f7541ad17e73c42088748a5219}} \\
\bottomrule
\end{tabularx}
\end{table}

\noindent These constants are implemented pre-launch candidates. They must be
benchmarked, independently reviewed, and frozen in a named release before
mainnet; changing them after launch is a hard fork. Block versions 2--8 and
...
unit tests and published known-answer vectors.
...
Free registration & \code{discrete-pq-free-reg-pow-v1} & yespower preimage \\
\midrule
...
\bibitem{bip39} M.~Palatinus, P.~Rusnak, A.~Voisine, and S.~Bowe.
\emph{BIP-39: Mnemonic code for generating deterministic keys.} 2013.
