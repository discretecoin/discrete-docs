...
The block ID intentionally hashes the unsigned header/tree blob and omits
$\sigma_B$; it is neither the DiscretePower result nor a hash of full block
serialization. Nodes still recompute the presented signature and PoW on
admission, so a header-only cache entry is not a validity verdict. Alternate
valid signatures are alternate proofs for the same candidate identity.
...
\item The genesis coinbase \code{extra} embeds a dated headline ---
  ...
  13~July 2026 02:39:16 UTC. Neither timestamp nor headline proves the public
...
the four DiscretePower tags are frozen transcript-version identifiers retained
