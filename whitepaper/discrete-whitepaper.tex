...
\vspace{0.7cm}
{\normalsize Protocol paper, revision 1.4 --- August 2026\par}
{\normalsize\itshape Mainnet live since 15 July 2026; consensus constants frozen. Independent review pending\par}
\vspace{0.7cm}
...
\noindent
Discrete is a live proof-of-work cryptocurrency whose transaction
validation is post-quantum from genesis; mainnet has been running since
15~July 2026. Ownership uses ML-KEM-768 for private
...
approximately 2\%-per-year tail. This paper specifies the running protocol,
wire formats, monetary policy, recovery procedure, residual risks, and the
limits that remain --- foremost among them independent review of the
composition.
\end{minipage}
...
amounts. Appendices~\ref{app:params}--\ref{app:wire} collect the frozen
network parameters, domain-separation tags, and wire formats.

This document describes the protocol as it runs on mainnet. Its constants match
the implementation, the specification files, and the pinned known-answer vectors
byte-for-byte; changing any of them now is a hard fork. What has \emph{not}
changed is the standing caveat: the composition is test-covered but not yet
independently audited, and any review should identify the exact repository
commit it examined.
...
implementation is a live, post-quantum, transparent-amount chain, with confidential-amount
...
consensus sizes are pinned in code and tests: a spend reveals a
...
\mathit{outContext} &= H\Bigl(\text{\code{discrete-pq-out-context-v2}}
  \concat \mathit{inputsHash} \concat \mathit{kemCt}
  \concat \mathrm{LE32}(i)\Bigr)
\label{eq:outcontext}
\end{align}
where $i$ is the output's index in the transaction. Uniqueness comes from
$\mathit{kemCt}$ (fresh per output: ML-KEM-768 encapsulation is randomized)
and $\mathit{inputsHash}$, never from $T$ --- $T$ is the routing index of
Section~\ref{sec:deposits} ($T{=}0$ for ordinary payments), and as of the
\emph{outContext-v2} revision it is deliberately \textbf{excluded} from the
context. Folding $T$ into the key-derivation input (as the original
\code{discrete-pq-out-context-v1} did) forced every receiver to enumerate
candidate $T$ values to find the one that decrypts, which in turn made
deposit-subaddress recovery depend on wallet-local bookkeeping (how many $T$
values had been issued) rather than the seed alone. Since $T$ never affects
consensus validation --- nodes never compute or check \emph{outContext}, they
only enforce the wire sizes it produces --- moving it out of the context is a
pure wallet-software change: $T$ now travels solely inside the AEAD-encrypted
payload below and is read back after a single decrypt attempt, so recovery
scales to any $T$ (sequential or arbitrary) at the same O(1) cost as $T{=}0$.
Outputs minted before this revision (always at $T{=}0$; no nonzero-$T$
deposit was ever issued under \code{-v1}) remain scannable through a
receiver-side legacy fallback using the original \code{-v1} domain tag and
formula; senders must never construct a new output that way.
...
function CreateOutput(inputsHash, i, a, unlockHeight, vpk, spk, T):
    ...
    ctx  $\leftarrow$ SHA3-256("discrete-pq-out-context-v2" $\|$ inputsHash $\|$ kemCt
                     $\|$ LE32(i))
...
Because \emph{outContext} excludes $T$ (Eq.~\eqref{eq:outcontext}), scanning
never enumerates candidate $T$ values: ML-KEM decapsulation and the AEAD
decrypt each run exactly once per output, and $T$ is read back from the
decrypted payload. For single-key-index wallets the candidate set is a single
shared spend public key; for aggregated-multikey addresses the single decrypt
result is tested against the primary and every known derived deposit spend
key. A receiver-side legacy fallback --- one extra decrypt attempt under the
original \code{discrete-pq-out-context-v1} formula at $T{=}0$ --- keeps
outputs minted before the \emph{outContext-v2} revision scannable; it is
tried only if the primary attempt fails. Wallet software may additionally
expose an off-by-default, operator-triggered recovery mode that brute-forces
the legacy formula across a $T$ window, in case an unupgraded or hand-rolled
sender ever constructs a legacy nonzero-$T$ output --- nothing in consensus
prevents that, since \emph{outContext} is never validated on-chain --- but no
such output has ever been observed.

\begin{algorithm}[tbp]
\caption{\textsc{Scan} --- recipient side, per transaction $\mathit{tx}$ with wallet spend-key candidates $\mathit{spks}$ (single-key-index: one shared key; aggregated-multikey: the primary plus known deposit keys).}
\label{alg:scan}
\begin{lstlisting}[style=pseudo]
function ScanTx(tx, vsk, spks):
    ...
    for each (i, out) in tx.outputs where out.type == 0x10:
        ...
        ctx $\leftarrow$ SHA3-256("discrete-pq-out-context-v2" $\|$ ih $\|$ out.kemCt $\|$ LE32(i))
        k   $\leftarrow$ HKDF(ss, "discrete-pq-aead-key-v1" $\|$ ctx, 32)
        m   $\leftarrow$ AEAD.Dec(k, $0^{12}$, aad=ctx $\|$ LE64(out.amount), out.encPayload)
        if m = $\perp$:
            // legacy fallback: pre-v2 outputs were always minted at T=0
            ctx $\leftarrow$ SHA3-256("discrete-pq-out-context-v1" $\|$ ih $\|$ out.kemCt
                            $\|$ LE32(i) $\|$ LE64(0))
            k   $\leftarrow$ HKDF(ss, "discrete-pq-aead-key-v1" $\|$ ctx, 32)
            m   $\leftarrow$ AEAD.Dec(k, $0^{12}$, aad=ctx $\|$ LE64(out.amount), out.encPayload)
            if m = $\perp$: continue      // not ours under either context, or tampered
        (rho, T) $\leftarrow$ parse m
        for each spk in spks:
            if SHA3-256("discrete-pq-spend-commit-v1" $\|$ spk $\|$ rho)
                    == out.spendCommit:
                record { txid, i, out.amount, out.unlockHeight, rho, ctx, T, spk }
                break          // ownership gate: never credit without this
\end{lstlisting}
\end{algorithm}
...
\item \textbf{CPU-oriented memory hardness.} \code{yespower-discrete}'s 16-MiB
  ...
  its hardware ratios remain advisable; the parameters are frozen on mainnet. The personalization tag is fixed and Discrete-specific, so
...
signature validation: it is a fixed network constant carrying the Treasury Reserve
...
validity and requires a coordinated consensus upgrade.
...
\item The 21 recipients are independent single-key post-quantum accounts held
  in cold storage; their spend public keys are committed in the frozen
...
\item The genesis coinbase \code{extra} embeds a dated headline ---
  ...
  that the genesis artifact could not have been assembled before its publication.
\end{itemize}
...
Adversarial simulation of these bounds remains outstanding.
...
Discrete is a live, post-quantum, transparent-amount chain.
...
the protocol assessment, not hidden behind absolute security claims. Mainnet has
been running since 15~July 2026 on the constants specified here; the
implementation is test-covered and still requires independent review.

% ==================================================================
\appendix

\section{Network parameters}\label{app:params}
...
\noindent These constants are the ones the live mainnet enforces; changing any
of them is a hard fork. Because \code{yespower-discrete} is a new
algorithm, independent cryptographic review and benchmarking remain advisable. Block versions 2--8 and
...
Output context (legacy, receiver fallback only) & \code{discrete-pq-out-context-v1} & SHA3-256 \\
Output context & \code{discrete-pq-out-context-v2} & SHA3-256 \\
