...
\vspace{0.7cm}
{\normalsize Pre-launch protocol paper, revision 1.1 --- July 2026\par}
...
Proof-of-work & DiscretePower & FIPS 202/204 + yespower-discrete & 16\,MiB scratchpad (draft) \\
...
history and resolvable by any node with no extra trust. Critically, the pair
names a \emph{position in one chain history}, not a cryptographic commitment to
a particular block or key set, so the number is only as stable as that position
is (see \emph{Resolution stability} below). It renders in
\textbf{H-I-C} form,
\[
...
Because the identifier is the position $(H,I)$ rather than a hash of the
registration, a reorganization that replaces block $H$ can change what the
number resolves to: the adopted branch may hold the same registration again,
hold \emph{no} registration (the number stops resolving), or hold a
\emph{different} identity's keys (the same text resolves to different public
keys). The registry rollback is intended consensus behavior, not a checksum or
database fault; a valid check character and a ``registered'' status attest only
consistent entry and current existence, never finality. The first-seen depth
limit (Section~\ref{sec:finality}, $\Phi=10$) sharply narrows this replacement
window but is local and history-relative, not a global certificate ---
partitioned, eclipsed, or newly syncing nodes can temporarily disagree on what
$(H,I)$ holds. Applications should therefore treat a freshly mined registration
as \emph{provisional} until it is at least $\Phi$ blocks behind the tip; for
high-value or long-lived use, wait $20$--$60$ confirmations, resolve through
independently operated nodes, and verify the resolved full address or key
fingerprint through a trusted channel. A sudden change in what an established
number resolves to is a stop condition, not an automatic address update. Owners
who need a durable public alias should publish a full PQ address or key
fingerprint alongside the account number.
...
IDs, closing the signature/PoW malleability that a signature-excluding ID would
admit; and because each child's $\mathit{prevHash}$ is its parent's ID, a
...
The block ID is an identifier only: consensus admission still verifies $\sigma_B$
and recomputes the DiscretePower PoW, never treating an ID cache hit as a
validity verdict. The 32-byte witness $W_B$ is retained separately so $\sigma_B$
can later be pruned while the ID stays recomputable.
...
$1.16$~GB/year. That signature is consumed exactly once --- to verify the
block's PoW at admission --- and a block buried beyond the finality depth
$\Phi$ (Section~\ref{sec:finality}) is never reorganized out by an honest node,
so its signature is never re-verified. The witness commitment is what makes
discarding it safe: a node may prune $\sigma_B$ for blocks older than $\Phi$
while retaining only the $32$-byte $W_B$, which keeps
$\mathrm{BlockID}(B)$ --- and therefore every child's $\mathit{prevHash}$ and
any checkpoint pin --- fully recomputable. This recovers about $99\%$ of the
per-block signature footprint ($3{,}309 \to 32$ bytes), giving up only the
ability to re-verify that ancient block's PoW, which is the ordinary pruning
trade and consistent with a node trusting its own validated history past
$\Phi$. The wire format and the identity construction already accommodate this,
so enabling pruning requires no consensus change; the reference implementation
retains full signatures today and treats pruning as a supported future
storage optimization rather than a shipped feature.
...
\item \textbf{CPU-oriented memory hardness.} \code{yespower-discrete}'s 16-MiB
  scratchpad ($N{=}4096$, $r{=}32$, draft) is the memory-hardness mechanism,
  targeting commodity CPUs rather than claiming hardware equality. Because it
  is a new yespower-derived algorithm, its
...
Block target & 90 s \\
Proof-of-work & DiscretePower: SHAKE-256 + ML-DSA-65 + yespower-discrete (signature-tape), $N{=}4096$, $r{=}32$ (16 MiB, draft) \\
Mining binding & coinbase recipient $=$ block signer (ML-DSA-65), Eq.~\eqref{eq:rhoc} \\
Difficulty & LWMA-1, window $N{=}60$, no cut/lag; median-time window 11; active future-time limit $3T$ \\
